function [tpr,fpr,precision,thresholds] = rocpr(targets,outputs)
...
if (numClasses == 1)
  ...
  [tpr,fpr,precision,thresholds] = rocpr(targets,outputs);
  tpr = tpr{1};
  fpr = fpr{1};
...
precision = fliplr(tpcount) ./ (fliplr(tpcount + fpcount));
